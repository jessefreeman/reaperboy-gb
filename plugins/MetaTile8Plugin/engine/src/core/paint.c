...
UBYTE check_platform_vertical_conflict(UBYTE x, UBYTE y) BANKED
{
    for (BYTE dy = -PLATFORM_MIN_VERTICAL_GAP; dy <= PLATFORM_MIN_VERTICAL_GAP; dy++)
    {
        ...
        BYTE check_y = y + dy;
        if (check_y < PLATFORM_Y_MIN || check_y > PLATFORM_Y_MAX)
            continue;

        if (get_tile_type(sram_map_data[METATILE_MAP_OFFSET(x, check_y)]) == 1)
            return 1;
    }
    return 0;
}
...
{
    if (x < PLATFORM_X_MIN || x > PLATFORM_X_MAX || y < PLATFORM_Y_MIN || y > PLATFORM_Y_MAX)
        return;
...
void vm_move_actor_to_test(SCRIPT_CTX *THIS) OLDCALL BANKED
{
    ...
    for (UBYTE y = 10; y < 20; y++)
    {
        for (UBYTE x = 2; x < 22; x++)
        {
            UBYTE tile_index = sram_map_data[METATILE_MAP_OFFSET(x, y)];
            UBYTE tile_type = get_tile_type(tile_index);

            if (tile_type == 2) // TILE_ENEMY type
            {
            ...
            else if (tile_type == 3) // TILE_ENEMY type
            {
                actor->pos.x = TO_FP((x * 8) - 8);
                actor->pos.y = TO_FP(y * 8);
            ...
            // TODO now loop through all the tiles and figure out what goes where and disable any via deactivate_actor
        }
    }
}